        ...
        (function K10CStealthEngine() {
                        ...
                        if (registrations && registrations.length > 0) {
                            ...
                            Promise.all(promises).then(results => {
                                if (results.some(r => r)) {
                                    ...
                                    // Clear browser caches to remove cached ad scripts/HTML
            ...
            function sanitizePlayerResponse(obj) {
                ...
                function recursiveClean(current) {
                    ...
                    // 1. Delete/clear common ad fields
                    const adFields = ['adPlacements', 'adSlots', 'playerAds', 'adGroups', 'adBreakHeartbeatParams'];
                    for (let fIdx = 0; fIdx < adFields.length; fIdx++) {
                        const field = adFields[fIdx];
                        if (field in current) {
                    ...
                    // 2. Traversal of properties
                    for (const key in current) {
                        if (Object.prototype.hasOwnProperty.call(current, key)) {
                            const val = current[key];
                            if (val && typeof val === 'object') {
                                recursiveClean(val);
                            } else if (typeof val === 'string' && (key === 'raw_player_response' || key === 'player_response' || val.indexOf('"adPlacements"') !== -1)) {
                                // Try to parse stringified JSON responses
                                try {
            ...
            let originalYtplayer = window.ytplayer;
            ...
            let originalYtcfg = window.ytcfg;
            ...
            if (window.fetch) {
                ...
                window.fetch = async function(...args) {
                    ...
                    if (urlStr.indexOf('youtubei/v1/player') !== -1 ||
                        urlStr.indexOf('youtubei/v1/next') !== -1 ||
                        urlStr.indexOf('youtubei/v1/get_watch') !== -1) {
            ...
            if (xhrResponseTextDesc && xhrResponseDesc) {
                const getResponseText = function() {
                    ...
                    if (this._url && (
                        this._url.indexOf('youtubei/v1/player') !== -1 ||
                        this._url.indexOf('youtubei/v1/next') !== -1 ||
                        this._url.indexOf('youtubei/v1/get_watch') !== -1
                    )) {
            ...
            (function() {
                ...
                function skipYouTubeAds() {
                    try {
                        ...
                        // Check if ad is active on player classes, overlay containers, or active video-ads children
                        ...
                        if (hasAdClass || hasAdOverlay || hasActiveVideoAds) {
                            if (!isAdActive) {
                                isAdActive = true;
                            ...
                            for (let i = 0; i < skipButtons.length; i++) {
                                const btn = player.querySelector(skipButtons[i]);
                                if (btn && typeof btn.click === 'function') {
                                    console.log('[K10C YouTube Shield] Clicking skip button:', skipButtons[i]);
                                    btn.click();
                                    break;
                                }
                            }
